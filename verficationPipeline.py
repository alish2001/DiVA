...
def id_check(camera, base_folder='Photos', selfie_file_name='selfie_file.jpg', id_file_name='id_file.jpg', selfie_time=5, id_time=10, break_timer=5):
    ...
    while True:
            ...
            cv2.putText(frame,"Get Ready For Selfie", (170, 200), cv2.FONT_HERSHEY_COMPLEX, 1.0, (0,0,255), 2)
            frame = cv2.putText(frame, "break will end after {0:.1f}s".format(break_timer - (curr_time-start_time)), (180, 50), cv2.FONT_HERSHEY_COMPLEX, 0.8, (0,0,255), 2)
            frame = camera.get_frame(frame)
            # writing image for thumbnail drawing shape
            yield (b'--frame\r\n'
                b'Content-Type: image/jpeg\r\n\r\n' + frame + b'\r\n\r\n')
    ...
    while True:
        ...
        if curr_time - start_time > selfie_time:
            ...
            break
        curr_time = time.time()
        frame = cv2.putText(frame, "picture will be taken after {0:.1f}s".format(selfie_time - (curr_time-start_time)), (180, 50), cv2.FONT_HERSHEY_COMPLEX, 0.8, (0,0,255), 2)
        frame = camera.get_frame(frame)
        # writing image for thumbnail drawing shape
        yield (b'--frame\r\n'
               b'Content-Type: image/jpeg\r\n\r\n' + frame + b'\r\n\r\n')
    ...
    while True:
            ...
            curr_time = time.time()
            cv2.rectangle(frame, (120, 120), (550, 300), (0,255,0), -1)
            cv2.putText(frame,'Get Ready For ID', (170, 200), cv2.FONT_HERSHEY_COMPLEX, 1.0, (0,0,255), 2)
            frame = cv2.putText(frame, "break will end after {0:.1f}s".format(break_timer - (curr_time-start_time)), (180, 50), cv2.FONT_HERSHEY_COMPLEX, 0.8, (0,0,255), 2)
            frame = camera.get_frame(frame)
            # writing image for thumbnail drawing shape
            yield (b'--frame\r\n'
    ...
    while True:
            ...
            curr_time = time.time()
            frame = cv2.putText(frame, "picture will be taken after {0:.1f}s".format(id_time - (curr_time-start_time)), (180, 50), cv2.FONT_HERSHEY_COMPLEX, 0.8, (0,0,255), 2)
            frame = camera.get_frame(frame)
            # writing image for thumbnail drawing shape
            yield (b'--frame\r\n'
                b'Content-Type: image/jpeg\r\n\r\n' + frame + b'\r\n\r\n')
    ...
    fc = faceRecognition()
    result = fc.detect_and_verify_faces(base_folder, selfie_file_name, id_file_name, draw=False)

    if result.is_identical:
        ret, frame = camera.read() # getting frame from camera
        cv2.rectangle(frame, (120, 120), (550, 300), (0,153,0), -1)
        cv2.putText(frame,'You Are VERIFIED!', (170, 200), cv2.FONT_HERSHEY_COMPLEX, 1.0, (0,0,255), 2)
        cv2.putText(frame,'PRESS Finish', (200, 260), cv2.FONT_HERSHEY_COMPLEX, 1.0, (0,0,255), 2)
        frame = camera.get_frame(frame)
    else:
        ret, frame = camera.read() # getting frame from camera
        cv2.rectangle(frame, (120, 120), (550, 300), (0,0,0), -1)
        cv2.putText(frame,'Verification Failed', (170, 200), cv2.FONT_HERSHEY_COMPLEX, 1.0, (0,0,255), 2)
        frame = camera.get_frame(frame)
